...
for indx in [0]:#np.arange(len(g_list)-1, len(g_list)-1 - 11, -1):
    ...
    for omega_gf2 in omega_gf2_list:
        ...
        filename = folder_path + f'/state_{omega_gf2:.0f}'
        ...
        Nss_list.append(np.abs(mean_n))
        F_list.append(np.abs(fano_number))

    print(g_list[indx][1], g_list[indx][0])
    print(Nss_list)
    np.savez(folder_path + f"/sol_{indx}_ss.npz", nss=Nss_list, fano=F_list)
